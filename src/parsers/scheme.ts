...
export const FunctionsForExplain = new Map<string,
    Function | number | undefined>([]);

const onlyALiteral = (tokens: Array<[string, TokenType]>) =>
    tokens.length === 2;
...
export class SchemeParser {
    ...
    buildSyntaxTree(input: string) {
            ...
            switch (currentToken[1]) {
                case TokenType.LeftParen && !functionExpected:
                    if (!functionExpected) {
                        ...
                        const node0: SyntaxNode = new SyntaxNode(
                            level.length,
                    ...
                    break;
                case TokenType.FunctionName:
                    const node1 = new SyntaxNode(
                        level.length,
                        SyntaxNodeType.Operator,
                        currentToken[0]
                    );
        ...
        if (!root) {
            throw new Error(
                `Unexpected Error for building syntax tree for ${input}, ${typeof input}`
            );
        }
            ...
            (n.children || [])
                .map(c => {
                    if (c.type === SyntaxNodeType.Literal) {
                        ...
                        if (
                            typeof newVar.value !== 'undefined' &&
                            typeof newVar.eval === 'function'
                        ) {
...
export class SyntaxNode {
    ...
    simplify(): any {
        ...
        return {
            depth: this.depth,
            children: this.children?.map(c => c.simplify()),
        };
    }
    ...
    eval(): number | Function | undefined {
        ...
        if (!this.children[0].eval()) {
            throw new Error(
                `Function not found! ${this.children[0].value}, ${
                    SyntaxNodeType[this.type]
                }`
            );
        }

        return (this.children[0].eval()! as Function).apply(
            null,
            this.children.slice(1)?.map(c => c.eval())
        );
    }
    ...
    define() {
        ...
        if (
            toBeDefined.type === SyntaxNodeType.Literal &&
            implementation.type === SyntaxNodeType.Literal
        ) {
            Functions.set(
                toBeDefined.value as string,
                implementation.value as number
            );
            return undefined;
        }
        ...
        Functions.set(
            fn.value as string,
            Functions.get(implementation.children[0].value as string)!
        );
    ...
    defineExplain() {
        ...
        FunctionsForExplain.set(
            fn.value as string,
            FunctionsForExplain.get(implementation.children[0].value as string)!
        );
    ...
    expand(): any {
        const evals = (c: SyntaxNode) => {
            ...
            if (String(res) === 'NaN') {
                console.error(`Evaluation error for ${c.toString()}`, c);
        ...
        if (this.children.slice(1).every(c => c.type === SyntaxNodeType.Literal)) {
            ...
            if (fn) {
                ...
                console.log('fn2 = ', this.children[0].value, Functions);
        ...
        res.children.slice(1).forEach((c, i) => {
            ...
            if (!FunctionsForExplain.get(c.children[0].value as string)) {
                res.children[i + 1] = new SyntaxNode(
                    1,
                    SyntaxNodeType.Literal,
                    c.eval() as number
                );
    ...
    flatten(): string {
        if (this.type === SyntaxNodeType.Literal) {
            return String(
                FunctionsForExplain.get(this.value as string) ?? this.value
            );
        }
    ...
    expandToEnd() {
        ...
        while (
            current &&
            current.type === SyntaxNodeType.Expression &&
            count < 100
            ) {
